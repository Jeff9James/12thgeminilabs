...
export class SearchApiService {
  ...
  async getIndexingJobStatus(videoId: string, jobId: string): Promise<IndexingJob> {
    const response = await apiClient.get<IndexingJob>(`/videos/${videoId}/index/status/${jobId}`);
    return response.data!;
  }
  ...
  async searchVideo(videoId: string, searchRequest: SearchRequest): Promise<SearchResponse> {
    const response = await apiClient.post<SearchResponse>(`/videos/${videoId}/search`, searchRequest);
    return response.data!;
  }
  ...
  async getPopularSearchTerms(videoId: string): Promise<string[]> {
    const response = await apiClient.get<string[]>(`/videos/${videoId}/search/popular`);
    return response.data || [];
  }

  // Re-index a video
  async reindexVideo(videoId: string): Promise<IndexingJob> {
    const response = await apiClient.post<IndexingJob>(`/videos/${videoId}/index/reindex`);
    return response.data!;
  }
