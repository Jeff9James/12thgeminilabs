...
export default function LearnHubPage() {
  ...
  // Live Mode state (Placeholder for Gemini Live API)
            ...
            <div className="mb-6 px-1">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`flex-1 px-3 py-2.5 text-xs font-bold rounded-lg transition-all border-2 ${useMetadata
        ...
        <div className="mt-auto p-6 border-t border-gray-50 bg-gray-50/50">
          <button
            onClick={() => {
              ...
              setRawResults([]);
              setQuery('');
              localStorage.removeItem('learn_hub_history');
              localStorage.removeItem('learn_hub_metadata');
            }}
          ...
          <div className="max-w-4xl mx-auto px-6 relative z-10">
            ...
            >
              ...
              <p className="text-xl text-purple-100 max-w-2xl mx-auto">
                Personalized tutoring and explanations from your study materials
              </p>

              {/* Start Live Lesson Button (Gemini Live API) */}
              <div className="mt-8">
                <button
                  onClick={() => setIsLiveMode(!isLiveMode)}
                  className={`inline-flex items-center gap-3 px-8 py-4 ${isLiveMode ? 'bg-red-500 hover:bg-red-600' : 'bg-gradient-to-r from-yellow-400 to-orange-500 hover:from-yellow-500 hover:to-orange-600'} text-white rounded-2xl font-bold text-lg shadow-xl hover:scale-105 active:scale-95 transition-all group`}
                >
                  <VideoIcon className={`w-6 h-6 ${isLiveMode ? 'animate-pulse' : 'group-hover:rotate-12 transition-transform'}`} />
                  {isLiveMode ? 'End Live Session' : 'Start Live Lesson'}
                  <div className="flex gap-1 ml-2">
                    <div className={`w-1.5 h-1.5 rounded-full ${isLiveMode ? 'bg-white' : 'bg-white/50'} animate-bounce [animation-delay:-0.3s]`} />
                    <div className={`w-1.5 h-1.5 rounded-full ${isLiveMode ? 'bg-white' : 'bg-white/50'} animate-bounce [animation-delay:-0.15s]`} />
                    <div className={`w-1.5 h-1.5 rounded-full ${isLiveMode ? 'bg-white' : 'bg-white/50'} animate-bounce`} />
                  </div>
                </button>
                <p className="text-purple-200 text-xs mt-3 font-medium opacity-80 uppercase tracking-widest">
                  ...
                  <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/10 space-y-6">
                    ...
                    <div className="pt-6 border-t border-white/10">
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() => setUseMetadata(true)}
                          className={`flex-1 px-4 py-2.5 text-sm font-bold rounded-lg transition-all ${useMetadata
            ...
            {chatHistory.length === 0 && (
              ...
              >
                <p className="text-xl text-purple-100">
                  Ask your AI Tutor or start a live lesson below
                </p>
              </motion.div>
            )}
            ...
            >
              {[
                'Explain the core concepts in my files',
                'Create a study guide for this material',
                'Quiz me on the main topics',
                'Elaborate on the most difficult section',
              ].map((example) => (
                <button
                  key={example}
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder={chatHistory.length === 0
                      ? "Ask your AI Tutor or start a lesson..."
